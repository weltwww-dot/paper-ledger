...
OA/arXiv、摘要和 PDF 阶段成果，避免人工撰写中文总结期间反复请求相同来源。去重库存取
`last_update.json`、当前 `data/papers.js` 与 `summaries/*.md` 的并集；等待 PDF 的中间批次也不会被再次报告为新增。
基准、本地 DOI/标题库存、期刊目录、抓取/PDF 实现发生变化，审计失败、日期变化或 JSON 损坏时会自动失效；
