...
2. 日常“更新”只用 `python scripts/run_update.py update`；中文六段式必须由当前执行 agent 对照原文亲自撰写，并经过 PDF 探测/下载/校验/证据登记、同步和六道闸门，不能只运行抓取步骤。同日续跑会安全复用已验证阶段；需要重抓时加 `--refresh`。
...
### 3.5 发布相关（本轮已完成）
- 前一轮路径说明提交 `37a40de` 已推送；本轮整改提交 `c1635df` 已推送至 `origin/main`。发布收尾标准提交 `ba30bd2` 也已推送，并已通过 Pages 当前提交构建与线上文件指纹校验。
- 后续有新变更且用户再次许可发布时执行：
  ```powershell
...
- 本次 `ba30bd2` 已按上述标准完成核验：Pages 构建提交匹配，线上论文数为 599，线上文件指纹与本地一致。
...
python -m unittest discover -s tests -v                 # 45 项
```
