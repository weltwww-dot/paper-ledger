...
神经架构搜索(NAS)旨在自动搜索高性能架构,并取得了相当大的进展.现有的NAS方法通过最小化平均案例验证损失来学习架构. 因此,搜索架构在最坏情况下做出正确预测的能力较低. 为了解决这一问题,我们提出了一个框架,利用深层基因模型生成对抗性验证实例,以衡量一个架构最坏的验证性能,并通过尽量减少生成的对抗性验证数据的损失来改进架构。 我们的框架基于多层次优化,它执行多个学习阶段的端到端。 对各种数据集的实验证明了我们的方法的有效性.
